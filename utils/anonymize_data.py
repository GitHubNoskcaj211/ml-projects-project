...
def anonymize_and_write(snowball, name, user_cols):
    ...
    df.to_csv(os.path.join(PUBLIC_ROOT, snowball, name), index=False)
...
for snowball in snowballs:
    if os.listdir(os.path.join(ORIGINAL_ROOT, snowball)) == []:
        continue
    os.makedirs(os.path.join(PUBLIC_ROOT, snowball), exist_ok=True)
