...
class GraphSAGE(torch.nn.Module, BaseGameRecommendationModel):
    ...
    def _train(self, debug=False):
        ...
        # Training numbers for consistency 
        loss_threshold = 0.35
        patience = 5
        patience_counter = 0
        prev_loss = float('inf')
        ...
        for epoch in tqdm(range(self.num_epochs), desc="Training"):
            ...
            if loss_change < loss_threshold:
                patience_counter += 1
                if (debug):
                    print(f"Epoch {epoch+1}, Loss: {loss.item()}, Minimal loss change detected.")
                if patience_counter >= patience:
                    print(f"Stopping early at epoch {epoch + 1} epochs with minimal loss improvement.")
                    break
            else:
                patience_counter = 0  # Reset counter if there's significant loss improvement
    ...
    def train(self, debug=False, user_node_ids=None):
        ...
        self.user_ids = (
            user_node_ids
            if user_node_ids is not None
            else self.data_loader.get_user_node_ids()
        )
        ...
        train_users_games_df = self.data_loader.users_games_df[
            self.data_loader.users_games_df["data_split"] == "train"
        ]
        edges = [
            (user, game)
            for user, game in zip(
                train_users_games_df["user_id"].tolist(),
                train_users_games_df["game_id"].tolist(),
            )
        ]
        edge_index_data = [
            (self.node_id_to_index[edge[0]], self.node_id_to_index[edge[1]])
            for edge in edges
        ]
        ...
        )  # Assuming self.hidden_channels is a list
        self.convs = torch.nn.ModuleList()
        self.convs.append(SAGEConv(self.in_channels, self.hidden_channels[0]))
        for i in range(1, self.num_layers):
            self.convs.append(
                SAGEConv(self.hidden_channels[i - 1], self.hidden_channels[i])
            )
    ...
    def score_and_predict_n_games_for_user(
        self, user_id, N=None, should_sort=True, games_to_include=[]
    ):
        ...
        return self.select_scores(
            scores,
            N=N,
            should_sort=should_sort,
            games_to_filter_out=games_to_filter_out,
            games_to_include=games_to_include,
        )
    ...
    def _fine_tune(
        ...
        if user_id not in self.node_id_to_index:
            raise ValueError(
                f"User ID {user_id} not found in node_id_to_index mapping."
            )
        else:
            user_index = self.node_id_to_index[user_id]

        new_game_indices = [self.node_id_to_index[game_id] for game_id in new_user_games_df["game_id"] if game_id in self.node_id_to_index]

        if new_game_indices:    # If user exists already 
            new_user_embedding = torch.mean(self.embeddings[new_game_indices], dim=0)
            self.embeddings[user_index] = new_user_embedding
