...
class CommonNeighborsModelLoadPredictEfficient(BaseGameRecommendationModel):
    ...
    def train(self):
        matrix = nx.adjacency_matrix(self.data_loader.train_network)
        self.index_to_node = list(self.data_loader.train_network.nodes())
        self.node_to_index = {node: ii for ii, node in enumerate(self.data_loader.train_network.nodes())}
        self.game_nodes = [node for node, data in self.data_loader.train_network.nodes(data=True) if data['node_type'] == NodeType.GAME]
...
class CommonNeighborsModelStorageMemoryEfficient(BaseGameRecommendationModel):
    ...
    def score_and_predict_n_games_for_user(self, user, N=None, should_sort=True):
        ...
        scores = [(game, user_scores[self.node_to_index[game]]) for game in self.game_nodes if game not in root_node_neighbors]
        return self.select_scores(scores, N, should_sort)
