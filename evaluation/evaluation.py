...
class Evaluator:
    ...
    def plot_embedding_percentile_absolute_error(self, embedding):
        self.metrics[f'{embedding}_absolute_error_percentiles_figure'] = get_percentile_figure(absolute_error(self.edge_results.loc[self.edge_results['expected_edge'] == True, f'predicted_{embedding}'], self.edge_results.loc[self.edge_results['expected_edge'] == True, f'expected_{embedding}']).values, f'Embedding {embedding} Absolute Error at Percentiles')
    
    def plot_embedding_percentile_squared_error(self, embedding):
        self.metrics[f'{embedding}_squared_error_percentiles_figure'] = get_percentile_figure(squared_error(self.edge_results.loc[self.edge_results['expected_edge'] == True, f'predicted_{embedding}'], self.edge_results.loc[self.edge_results['expected_edge'] == True, f'expected_{embedding}']).values, f'Embedding {embedding} Squared Error at Percentiles')
    ...
    def get_user_positional_errors(self):
        ...
        filtered_rows = self.edge_results.loc[self.edge_results['expected_edge'] == True]
    ...
    def plot_user_based_roc_curve(self):
        max_predicted_score = np.max(self.edge_results['predicted_score'])
        ...
        roc_curve = skmetrics.roc_curve(self.edge_results['expected_edge'].astype(int).tolist(), 1 / (self.edge_results['user_predicted_rank'] + transformed_scores))
